(function(){
  ...
  /* живые отзывы из Telegram (reviews.json) — грузим только после полной
     загрузки страницы, чтобы не мешать критичным ресурсам, и с таймаутом,
     чтобы не зависать, если backend временно недоступен */
  (function(){
    ...
    function loadReviews(){
      ...
      fetch('https://reviews.documenti.ge/reviews.json', ctrl?{signal:ctrl.signal}:{})
        .then(function(r){ if(timer)clearTimeout(timer); return r.ok ? r.json() : []; })
        .then(function(items){
      ...
      .catch(function(){ if(timer)clearTimeout(timer); /* если не удалось — просто остаются статичные отзывы */ });
    }
    if(document.readyState==='complete'){ setTimeout(loadReviews, 0); }
    else{ window.addEventListener('load', function(){ setTimeout(loadReviews, 0); }); }
  })();
  ...
  function enhanceForm(form){
    ...
    if(form.classList.contains('partner-form')){
      ...
      if(contactElP && !form.querySelector('.f-ctype')){
        ...
        selP.setAttribute('aria-label','Тип контакта');
    ...
    if(svcEl){
      ...
      if(nameElForSvc && consentElForSvc){
        ...
        svcSelect.className='f-service';svcSelect.setAttribute('aria-label','Выберите услугу');
    ...
    if(nameEl && !form.querySelector('.f-city')){
      ...
      citySel.className='f-city';citySel.setAttribute('aria-label','Выберите город');
...
/* предзагрузка страниц при наведении/касании — к моменту клика страница уже готова */
(function(){
  var done={};
  ...
  function onHover(e){
    ...
    try{
      ...
      if(u.origin===location.origin && u.pathname!==location.pathname) prefetch(a.href);
    }catch(err){}
  }
